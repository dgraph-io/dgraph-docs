...
Alternatively, you can use the `--vault_*` options to enable encryption as explained below.
...
If multiple Dgraph Alpha nodes are part of the cluster, you will need to pass the `--encryption_key_file` flag or the `--vault` superflag to
each of the Dgraph Alpha nodes.

Once an Dgraph Alpha has encryption enabled, the encryption key must be provided in order to start a Dgraph Alpha node.
If a Dgraph Alpha node restarts, the `--encryption_key_file` flag or the `--vault` superflag must be set along with the key in order to
restart successfully.
