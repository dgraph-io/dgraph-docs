...
* The `state`  query returns the current state of the cluster and group membership information. For more information about `state` see [here](/graphql/[deploy/admin/dgraph-zero#more-about-the-state-endpoint](deploy/admin/dgraph-zero#more-about-the-state-endpoint)).
