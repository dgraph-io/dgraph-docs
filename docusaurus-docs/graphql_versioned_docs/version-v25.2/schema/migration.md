...
2. Just use the [`@dgraph(type: ...)`](directives/directive-dgraph) directive to maintain backward compatibility 
...
2. Just use the [`@dgraph(pred: ...)`](directives/directive-dgraph) directive to maintain backward compatibility
   with the existing data.
