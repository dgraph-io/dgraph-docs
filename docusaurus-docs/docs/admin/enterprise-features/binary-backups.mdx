...
the backup. Unlike [exports](/dgraph-overview/dgraph-administration#export-database),
...
[whitelisted for admin operations](/dgraph-overview/dgraph-administration#whitelisting-admin-operations).
...
Dgraph's [Encryption at Rest](/dgraph-overview/admin/enterprise-features/encryption-at-rest) uses a symmetric-key
...
You use the `--vault` [superflag](/dgraph-overview/cli/superflags) to specify the [Hashicorp Vault](https://www.vaultproject.io/) server address (`addr`), role id (`role-id-file`), secret id (`secret-id-file`) and the field that contains the encryption key (`enc-field`) that was used to encrypt the backup.
