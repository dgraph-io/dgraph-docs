...
If you are interested in clients for GraphQL endpoint, please refer to [GraphQL clients](/dgraph-overview/graphql-clients) section.
...
- write a value that conflicts on an index for a predicate with `@upsert` set in the schema (see [upserts](/dgraph-overview/dql/upserts)).
